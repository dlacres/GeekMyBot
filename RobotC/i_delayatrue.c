// As a programmer I want to make sure a signal is true so that a noise does not trigger my signal
bool DelayATrue(bool signal,int delay, int &count){
	if(signal)
		count++;
	else
		count=0;
...
task main(){
	...
	while(i<20){
		...
		out=DelayATrue((bool)in, size, count);
